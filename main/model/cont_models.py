...
import main.model.laplacian_builders as lb
from main.model.sheaf_models import LocalConcatSheafLearner, EdgeWeightLearner
...
class GeneralSheafDiffusion(SheafDiffusion):
    ...
    def __init__(self, edge_index, args):
        ...
        self.phi_1 = nn.Linear(self.hidden_dim, self.hidden_dim)
        self.phi_2 = nn.Linear(self.hidden_dim, self.hidden_dim)
        
        self.kappa_1 = nn.Linear(self.hidden_dim, self.hidden_dim)
        self.kappa_2 = nn.Linear(self.hidden_dim, self.hidden_dim)
    ...
    def forward(self, x, adj, y_a):
        ...
        '''x_d =  torch.tanh(sparse.mm(self.sparse_mm(adj, y_a), x) - sparse.mm(self.sparse_mm(adj, 1-y_a), x))
        e = self.kappa_1(x)+x + 1e-8
        x = x + torch.sigmoid(self.beta)* torch.tanh((self.phi_1(x)*x) / e )
        e = self.kappa_2(x_d)+torch.abs(x_d)  + 1e-8
        x = x + torch.sigmoid(self.gamma)*torch.tanh((self.phi_2(x_d)*x_d) / e )'''
